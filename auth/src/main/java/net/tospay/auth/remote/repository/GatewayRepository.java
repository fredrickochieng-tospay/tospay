...
public class GatewayRepository {
    ...
    public LiveData<Resource<List<Country>>> countries() {
    ...
    public LiveData<Resource<Result>> topup(String bearerToken, String type, String accountId, String amount) {
        ...
        return new NetworkBoundResource<Result, Result>(mAppExecutors) {
            ...
            @Override
            protected LiveData<ApiResponse<Result>> createCall() {
                return mGatewayService.topup(bearerToken, request);
            }
        }.asLiveData();
    }

    public LiveData<Resource<WalletTransactionResponse>> fetchWalletTransactions(String bearerToken) {
        return new NetworkBoundResource<WalletTransactionResponse, Result<WalletTransactionResponse>>(mAppExecutors) {

            private WalletTransactionResponse resultsDb;

            @Override
            protected void saveCallResult(@NonNull Result<WalletTransactionResponse> item) {
                resultsDb = item.getData();
            }

            @Override
            protected boolean shouldFetch(@Nullable WalletTransactionResponse data) {
                return true;
            }

            @NonNull
            @Override
            protected LiveData<WalletTransactionResponse> loadFromDb() {
                if (resultsDb == null) {
                    return AbsentLiveData.create();
                } else {
                    return new LiveData<WalletTransactionResponse>() {
                        @Override
                        protected void onActive() {
                            super.onActive();
                            setValue(resultsDb);
            ...
            @NonNull
            @Override
            protected LiveData<ApiResponse<Result<WalletTransactionResponse>>> createCall() {
                return mGatewayService.fetchWalletTransactions(bearerToken);
            }
        }.asLiveData();
    }

    public LiveData<Resource<Result>> saveFcmToken(String bearerToken, String fcmToken) {
        ...
        return new NetworkBoundResource<Result, Result>(mAppExecutors) {
            ...
            @Override
            protected LiveData<ApiResponse<Result>> createCall() {
                return mGatewayService.saveFcmToken(bearerToken, request);
            }
        }.asLiveData();
    }
}
